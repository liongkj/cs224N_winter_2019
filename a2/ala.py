class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
            ...
            # @returns enc_hiddens (Tensor): Tensor of hidden units with shape (b, src_len, h*2), where b = batch size, src_len = maximum source sentence length, h = hidden size.
            ...
            ###         src_len = maximum source sentence length, b = batch size, e = embedding size. Note that there is no initial hidden state or cell for the decoder.
            ...
            X = self.model_embeddings.source(source_padded) # get source tensor from model_embedding.ipynb
            X = pack_padded_sequence(X,source_lengths) #pack means identified which are padding, optimized computation

            enc_hiddens, last_hidden, last_cell = self.encoder(X) #go thru encoder layer
            enc_hiddens, _ = pad_packed_sequence(enc_hiddens) #inverse for pack
            enc_hiddens = enc_hiddens.permute(1,0,2) #permute means rotate the tensor but keep the orders of shape

            #last hidden shape (2,b,h) 2 = 2 list
            #torch.cat(array, dimension)
            init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0],last_hidden[1])),1)
            inti_decoder_cell = self.c_projection(torch.cat((last_hidden[0],last_hidden[1])),1)
            dec_init_state = (init_decoder_hidden, init_decoder_cell) #combine both as tuple
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        @returns enc_masks (Tensor): Tensor of sentence masks of shape (b, src_len), where src_len = max source length, h = hidden size.
        """
