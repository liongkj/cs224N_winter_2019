...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # get source tensor from model_embedding.ipynb
        X = self.model_embeddings.source(source_padded)
        # pack means identified which are padding, optimized computation
        ...
        enc_hiddens, (last_hidden, last_cell) = self.encoder(
            X)  # go thru encoder layer
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)  # inverse for pack
        # permute means rotate the tensor but keep the orders of shape
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor, dec_init_state: Tuple[torch.Tensor, torch.Tensor], target_padded: torch.Tensor) -> torch.Tensor:
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = torch.squeeze(Y_t, 0)  # squeeze to remove 1's
